// src/main/java/com/messdiener/cms/liturgy/app/helper/LiturgieHelper.java
package com.messdiener.cms.liturgy.app.helper;
...
public class LiturgieHelper {
    ...
    public void extractedLoadMethod(Model model, Optional<String> startDateS, Optional<String> endDateS, Person person, Optional<Person> target) throws SQLException {
        ...
        if (target.isEmpty()) {
            ...
            // Fix: Person -> DTO ohne nicht existente PersonService.getPersonDTO(...)
            persons = List.of(targetPerson.toPersonOverviewDTO());
        }
    ...
    public List<EventParticipationDto> getEventParticipation(Optional<String> startDateS, Optional<String> endDateS, Person person, Optional<Person> target) throws SQLException {
        ...
        return liturgieList.stream()
                .map(l -> {
                    ...
                    long dutyCount       = ps.values().stream().filter(s -> s == LiturgieState.DUTY).count();
                    long availableCount  = ps.values().stream().filter(s -> s == LiturgieState.AVAILABLE).count();
                    long unavailableCount= ps.values().stream().filter(s -> s == LiturgieState.UNAVAILABLE).count();
