...
public class FinanceController {
    ...
    public String finance(HttpSession session, Model model, @RequestParam("startDate")Optional<String> startDateS,  @RequestParam("endDate")Optional<String> endDateS, @RequestParam("q")Optional<String> q, @RequestParam("id")Optional<String> idS) throws SQLException {
        ...
        model.addAttribute("persons", personService.getActivePersonsByPermissionDTO(person.getFRank(), person.getTenant()));
        ...
        model.addAttribute("transactions", (Map<FinanceEntry, PersonOverviewDTO>) financeService.getFinancePersonMap(person.getTenant()));

        List<StorageFile> files = storageService.getFiles(person.getId(), FileType.TRANSACTION);
        model.addAttribute("files", files);

        double cash = financeService.getSumOfRevenueCashByTenant(person.getTenant()) - financeService.getSumOfExpenseCashByTenant(person.getTenant());
        double account =  financeService.getSumOfRevenueAccountByTenant(person.getTenant()) -  financeService.getSumOfExpenseAccountByTenant(person.getTenant());
    ...
    public RedirectView save(@RequestParam("date")String dateE, @RequestParam("incomeCash")double incomeCash, @RequestParam("expenseCash")double expenseCash,
        ...
        FinanceEntry financeEntry = new FinanceEntry(UUID.randomUUID(), person.getTenant(), receipt, personId, 0, CMSDate.convert(dateE, DateUtils.DateType.ENGLISH), incomeCash, expenseCash, incomeBank, expenseBank, title, category, note);
